from CementStrength.entity import DataIngestionConfig,DataIngestionArtifact
import sys, os
...
class DataIngestion:
    ...
    def download_concrete_data(self)-> str:
        try:
            ...
            raw_file_path = os.path.join(raw_data_dir,
                                raw_file_name)
            logger.info(f"Downloading [{raw_file_name}] from [{download_url}] to [{raw_data_dir}]")
            urllib.request.urlretrieve(download_url, raw_file_path)
            logger.info(f"Downloaded [{raw_file_name}] successfully.")
        except Exception as e:
            logger.exception(e)
    ...
    def __del__(self):
        logger.info(f"{'='*20}Data Ingestion log Ended{'='*20} \n\n")